...
%macro single_year(year);
	...
	DATA DCvacant_&year._1 ;
		SET Ipums.Acs_&year._vacant_DC ;
	RUN;

	DATA DCarea_&year.;
		SET Ipums.Acs_&year._dc;
	RUN;

	PROC SORT DATA = DCvacant_&year._1;
		BY upuma;
	RUN;

	PROC SORT DATA = DCarea_&year._1;
		BY upuma;
	RUN;

	/* unneeded code from NC project
