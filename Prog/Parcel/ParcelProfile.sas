/**************************************************************************
 ...
 Modifications:
	12/8/14 - MW - Modified to address the descrepancy between owner-occupied units and parcels. Rental units are taken from DHCD.Units_regression;
...
data parcel_base_oo
	(keep=ssl parcel_base ui_proptype address1 address2 address3 ui_proptype premiseadd hstd_code assess_val no_units sf_unit condo_unit coop_unit rental_build other assess_val_sf_lt100k 
	...
	set realprop.parcel_base (where=(ui_proptype in ("10","11","12","13","19") and in_last_ownerpt=1));
	parcel_base=1;
...
proc datasets lib=work;
	modify parcel_base_oo;
	attrib _all_ format=;
	run;

data parcel_rental_units_oo
	(keep=ssl ui_proptype parcel_rental units_active units_full);
	set dhcd.units_regression;
	parcel_rental=1;
run;
...
proc sort data=parcel_base_oo
	out=sort_parcel_base;
	by ssl;
	run;

proc sort data=parcel_rental_units_oo
...
%create_own_occ(
  inds=  parcel_profile,
  outds=  parcel_profile_test,
  inlib= work,
  outlib=  work
);

**Edits on adj_unit_count**
For single-family, unit count=1
	condominiums, unit count=1
	co-ops, unit count = no_units
	rental, unit count = units_full
	Other, unit count = adj_unit_count;

data parcel_profile_test_unitct;
	set parcel_profile_test;
	if ui_proptype=10 then adj_unit_count_new=1;
		else if ui_proptype=11 then adj_unit_count_new=1;
		else if ui_proptype=12 then adj_unit_count_new=no_units;
		else if ui_proptype=13 then adj_unit_count_new=units_full;
		else adj_unit_count_new=adj_unit_count;
	run;

data dmped.parcel_profile;
	set parcel_profile_test_unitct (where=(parcel_base=1 or prescat=1));
	if owner_occ_sale=1 then do;
		owner_occ=1;
		renter_occ=adj_unit_count_new-owner_occ;
		end;
	else if owner_occ_sale=0 then do;
		owner_occ=0;
		renter_occ=adj_unit_count_new-owner_occ;
		end;
	if owner_occ_sale=1 then owner_occ_parcel=1;
		else owner_occ_parcel=0;
	if owner_occ_sale=0 then renter_occ_parcel=1;
		else renter_occ_parcel=0;
	label 	sf_unit					='Count: Single Family Units'
			...
			owner_occ				='Number of Owner Occupied Units'
			...
			adj_unit_count_new			='New Adj. Unit Count'
			owner_occ_parcel			='Count: Owner Occupied Parcel'
			renter_occ_parcel			='Count: Renter Occupied Parcel'
;
	run;

	data test;
	set dmped.parcel_profile (where=(ui_proptype='13'));
	run;
...
%macro Summarize( level= );
  ...
  ** Summarize by specified geographic level all variables except for rent control **;
  ...
  proc summary data=dmped.parcel_profile nway completetypes;
    ...
    output 
      ...
      sum (units_active sub_all_proj Sub_CDBG_proj Sub_HOME_proj Sub_HPTF_proj Sub_LIHTC_proj Sub_McKinney_proj 
		...
		assess_val_condo_lt100k assess_val_condo_100k assess_val_condo_200k assess_val_condo_300k assess_val_condo_400k
		assess_val_condo_500k assess_val_condo_600k assess_val_condo_700k assess_val_condo_800k assess_val_condo_900k assess_val_condo_1mil
		adj_unit_count_new owner_occ_parcel renter_occ_parcel)=
	median=
	  mean=
	  ;
  ...
  run;
	...
	proc summary data=dmped.parcel_profile n sum;
	class &level /preloadfmt;
      format &level &level_fmt;
	class ui_proptype;
	var adj_unit_count_new;
	output
		out=dmped.parcel_profile_unitct_&filesuf
		sum (adj_unit_count_new)=;
	run;
...
proc means data=dmped.parcel_profile (where=(ui_proptype='13')) n sum;
	class owner_occ_sale;
	var adj_unit_count_new renter_occ owner_occ;
	run;
