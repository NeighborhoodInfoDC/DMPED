...
%File_info( data=parcel_profile_test_unitct, freqvars=ui_proptype Owner_occ_sale )
...
proc tabulate data=dmped.parcel_profile format=comma8.0 noseps missing;
  ...
  table 
    ...
    all='Total' ui_proptype,
    /** Columns **/
    sum=' ' *adj_unit_count_new='New Adjusted Unit Count (adj_unit_count_new)' * (all='Total' Owner_occ_sale)
    /rts=40
  ;
