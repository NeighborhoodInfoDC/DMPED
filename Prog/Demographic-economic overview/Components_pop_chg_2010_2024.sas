/**************************************************************************
 Program:  Components_pop_chg_2010_2024.sas
 Library:  DMPED
 ...
 2024 estimates - State level only
...
** Read 2020 - 2024 populations components data **;

filename fimport "&_dcdata_r_path\Census\Raw\Census population estimates\NST-EST2024-ALLDATA.csv" lrecl=5000;

proc import out=NST_est2024
    datafile=fimport
    dbms=csv replace;
  datarow=2;
  getnames=yes;
...
/*%File_info( data=NST_est2024, printobs=0 )*/

run;

proc print data=NST_est2024;
  where state = 11;
  id state;
  var POPESTIMATE2020 BIRTHS2020 DEATHS2020 INTERNATIONALMIG2020 DOMESTICMIG2020;
  title2 'NST_est2024';
...
data Components;
  ...
  merge 
    Co_est2020 (drop=region division NATURALINC2020 where=(state in (11) and county in (1)))
    NST_est2024 (drop=region division where=(state in (11)));
  by state;
...
data Components_tr;
  ...
  keep Year totalchg naturalchg internationalmig domesticmig netmig;
...
ods csvall body="&_dcdata_default_path\DMPED\Prog\Demographic-economic overview\Components_pop_chg_2010_2024.csv";
