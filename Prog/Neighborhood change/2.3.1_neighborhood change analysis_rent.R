...
summary_rentcat <- master_rent %>% 
  ...
  summarise(renter_2022=sum(renter_2022),
            ...
            pctchangexx=mean(pctchg_lowincome, na.rm=TRUE),
            pctchangeblk=mean(pctchange_2012_2022_blk, na.rm=TRUE),
            pct_college=mean(pct_college_2022),
            homeprice=mean(medianhome_2022, na.rm=TRUE),
...
test <- master_rent %>% 
  ...
  select(total_hh_2022)

write.csv(summary_rentcat, "C:/Users/Ysu/Box/Greater DC/Projects/DMPED Housing Assessment 2024/Task 2 - Nbrhd Change and Displacement Risk Assessment/Data collection/Clean/summary_bytype_rentcat.csv")

  
type <- master_rent %>% 
    select(GEOID, NBH_NAMES, neighborhoodtype) %>% 
    st_drop_geometry()
  
write.csv(type,"C:/Users/Ysu/Box/Greater DC/Projects/DMPED Housing Assessment 2024/Task 2 - Nbrhd Change and Displacement Risk Assessment/Data collection/Clean/neighborhoodtype_rentcat.csv")
...
ggplot() +
  ...
  coord_sf(datum = NA)+
  labs(title = paste0("Neighborhood Change in DC Based on Rent Level Change"),
