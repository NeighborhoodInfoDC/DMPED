...
#these are output from compile analysis data,R, context data and usps vacancy data pull requests
...
map_file <- merge(analysismaster,tractboundary_20, by=c("GEOID")) %>% 
  ...
  filter(!GEOID %in% c("11001007301","11001010900")) %>% #drop Joint Base Anacostia-Bolling
  st_as_sf() 
...
master6 <- map_file %>% 
  ...
  filter(!is.na(medianhome_2022)& !is.na(medianhome_2012_2020)) %>% #given the new method no longer exclude tracts without 2000 data
...
test <- master6 %>% 
  ...
  summarise(hh=sum(total_hh_2022))
...
#prediction - see 6.2 final tables program, these regression based predictions are no longer used
# 
...
# target_var <- "changeinblack"
# predictors <- c("distance", "distancesquared", "vacancy", "changerent", "changeunits", 
#                 "hcv", "lowincjob", "black", "homevalue", "pct_renter")
# 
# # Initialize an empty data frame to store the results
# results <- data.frame(Model = character(), Adjusted_R_squared = numeric(), CV_RMSE = numeric(), 
#                       AIC = numeric(), Variables = character())
# 
# # Set up cross-validation control with 10-fold CV repeated 10 times
...
#     
#     # Fit the model using 10-fold cross-validation repeated 10 times
#     model_cv <- train(formula, data = predictionmaster1, method = "lm", trControl = train_control)
#     
#     # Extract cross-validated RMSE (average over all repetitions)
#     cv_rmse <- mean(model_cv$results$RMSE)
#     
#     # Fit the model on the entire dataset to calculate Adjusted R-squared and AIC
...
# }
# 
# # Filter for the best model based on lowest CV RMSE, and lowest AIC
# best_model <- results %>%
#   arrange(CV_RMSE, AIC) %>%
#   slice(1)
# 
...
# blackmodel <- lm(changeinblack ~distancesquared+vacancy+changeunits+black+pct_renter, data = predictionmaster1)
# summary(blackmodel)
# 
# #################predict
# 
...
# # lowincmodel <- lm(changeinlowinc ~ changeunits+hcv+homevalue+pct_renter, data = predictionmaster1)
# lowincmodel <- lm(changeinlowinc ~ distance+hcv+black+pct_renter, data = predictionmaster1)
# 
# summary(lowincmodel)
# 
...
# 
# test <- prediction_blackpop %>% 
#   group_by(predictiontype_black) %>% 
#   count()
#   
# 
